...
Em um cenário em que $M < N$, temos $\rank{\bvR[T]{i} \bvR{i}} = \rank{\bvR{i}} = R \leq M$, e como $\bvR[T]{i} \bvR{i} \in \re^{\sz{N}{N}}$, o produto é uma matriz deficiente em posto, sendo portanto garantidamente não invertível. Para o método padrão estabelecido na literatura (\cref{subsec:sec2:standard_ttt_approach}), isso força o uso de uma matriz de regularização ou o uso de solucionadores de gradiente nulo sem inversão. Para a inversão baseada em SVD proposta, destaca-se que inclusive assume-se que $R < \min(M,N)$. Para a estrutura proposta, $M < N$ garante que $N-R > 0$, e com isso a existência de espaço-nulo para $\bvR{i}$, assegurando a possibilidade de uma minimização em $\mu$ por meio da etapa proposta de exploração do espaço-nulo.
