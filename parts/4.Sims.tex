...
To compare in practice the techniques developed throughout the text, simulated environments were created, in which these techniques were employed. Namely, we will compare the literature standard technique from \cref{eq:sec2:solution_regularization-problem_simple} with the proposed method as obtained in \cref{eq:sec2:definition_zi+1_with_mu} with truncation. In this section, we present the environment and practical conditions assumed, detail the considerations taken for the exhibited techniques, as well as other underlying necessary parameters, methods and metrics; and finally, we bring forth the results, and their following discussions.
...
This allows us to compare the estimated models to the observed result through the objective metric that is being optimized.
...
These results show that both the traditional/literature and the proposed SVD-based approaches have very similar results. Other simulations (not shown here) reinforce that, with a proper choice of parameters for both techniques, the results are always only marginally different.
